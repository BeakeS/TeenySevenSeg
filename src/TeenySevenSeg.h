...
class TeenySevenSeg {
  ...
  public:
    ...
    void drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {
      _displayObj.drawLine(x0, y0, x1, y1, color);
    }
  ...
  private:
    ...
    void drawSSSegStripe(segName_t segName, int16_t n, int16_t x, int16_t y, int16_t l, int16_t w, uint16_t color) {
      ...
      switch(segName) {
        ...
        case SegB : segInfo={SegV, l-1-n, 0           }; break;
        case SegC : segInfo={SegV, l-1-n, l-w         }; break;
        case SegD : segInfo={SegH, 0,     (2*l)-w-1-n }; break;
        case SegE : segInfo={SegV, n,     l-w         }; break;
        case SegF : segInfo={SegV, n,     0           }; break;
        case SegG : segInfo={SegH, 0,     l-1-n       }; break;
      ...
      if(segName==SegG) {
        switch(w) {
          case 1  : segClip=1; break;
          case 2  : segClip=1; break;
          case 3  : segClip=max(w-1-n, n); break;
          default : segClip=max(w-1-n, n); break;
        }
      } else {
        switch(w) {
          case 1  : segClip=1; break;
          case 2  : segClip=w-1-n; break;
          default : segClip=max(1, w-1-n); break;
        }
      }
      if(segInfo.segDir==SegH) {
        ...
        //}
        drawLine(x+segInfo.xOffset+segClip,     y+segInfo.yOffset,
                 x+segInfo.xOffset+l-1-segClip, y+segInfo.yOffset,
                 color);
      } else {
        ...
        //}
        drawLine(x+segInfo.xOffset, y+segInfo.yOffset+segClip,
                 x+segInfo.xOffset, y+segInfo.yOffset+l-1-segClip,
                 color);
      }
    }
    ...
    void drawSSColon(int16_t x, int16_t y, int16_t l, int16_t cl, int16_t cw, uint16_t color) {
      ...
      for(int16_t i=0; i<2; i++) {     //i=segment count
        for(int16_t j=0; j<cw; j++) {   //j=stripe count
          ...
          //}
          drawLine(x+segInfo.xOffset, y+segInfo.yOffset+colonOffset,
                   x+segInfo.xOffset, y+segInfo.yOffset+colonOffset+cl-1,
                   color);
        }
      }
    }
